...
public class AttributeCharacterizer extends Characterizer {
	...
	public static final String[] ids = new String[] { "ValuesCount", "NonMissingValuesCount", "MissingValuesCount", "Distinct", "AverageClassCount",
			...
			"Entropy", "JointEntropy", "MutualInformation", "RequiredEquivalentAtts", "NoiseToSignalRatio",
	...
	public Map<String, Double> characterize(Instances dataset) {
		...
		// Percentages
		MissingValues = (MissingValuesCount > 0 ? 1.0 : 0.0);
		...
		qualities.put("RequiredEquivalentAtts", EquivalentNumberOfAtts);
		...
		if (attribute.isNumeric()) {
			...
			qualities.put("HasPositiveValues", (PositiveCount > 0 ? 1.0 : 0.0));
			qualities.put("HasNegativeValues", (NegativeCount > 0 ? 1.0 : 0.0));
		}
		...
		if (attribute.isNominal()) {
			...
			if (index != dataset.classIndex() && Distinct > 1 && classValuesTab.length > 2) {
